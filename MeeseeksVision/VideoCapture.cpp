...
void VideoCapture::Finish(const char *pPathAndFile) {
   //DELAYED FRAMES
   m_pathAndFile = pPathAndFile;
   AVPacket pkt;
   av_init_packet(&pkt);
   pkt.data = NULL;
   pkt.size = 0;
...
void VideoCapture::Remux() {
   ...
   if ((err = avformat_alloc_output_context2(&ofmt_ctx, NULL, NULL, m_pathAndFile.c_str()))) {
      Debug("Failed to allocate output context", err);
      goto end;
   }
   ...
   if (!(ofmt_ctx->oformat->flags & AVFMT_NOFILE)) {
      if ((err = avio_open(&ofmt_ctx->pb, m_pathAndFile.c_str(), AVIO_FLAG_WRITE)) < 0) {
